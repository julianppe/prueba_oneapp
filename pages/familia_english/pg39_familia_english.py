...
dash.register_page(__name__,
                   path='/no-neighbors-homosex',  # represents the url text
                   name='Adult population who would not like to have homosexual neighbors',  # name of page, commonly used as name of link
                   title='Adult population who would not like to have homosexual neighbors'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/familia_english/no_vecinos_homosex.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page39_familia_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page39_familia_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
            ranger_slider_year_familia_english,
        ], width=12),
    ]),
])
...
@callback(
    Output('page39_familia_english-line', 'figure'),
    Input('all-pages-dropdown-pais-familia-english', 'value'),
    Input('page39_familia_english-comparacion_por_elect', 'value'),
