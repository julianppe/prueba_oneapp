...
dash.register_page(__name__,
                   path='/without-contraception-access-mics',  # represents the url text
...
# page 1 data
df = pd.read_csv("datasets/familia_english/anticoncepcion_sin_acceso_mics.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Women', id='page34_familia_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page34_familia_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page34_familia_english-line', 'figure'),
    Input('all-pages-dropdown-pais-familia-english', 'value'),
    Input('page34_familia_english-comparacion_por_elect', 'value'),
