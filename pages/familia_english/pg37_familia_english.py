...
dash.register_page(__name__,
                   path='/partner-adult',  # represents the url text
                   name='Percentage of adults in a couple',  # name of page, commonly used as name of link
                   title='Percentage of adults in a couple'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/familia_english/pareja_adultos.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page37_familia_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page37_familia_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page37_familia_english-line', 'figure'),
    Input('all-pages-dropdown-pais-familia-english', 'value'),
    Input('page37_familia_english-comparacion_por_elect', 'value'),
