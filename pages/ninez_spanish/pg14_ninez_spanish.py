...
dash.register_page(__name__,
                   path='/puntajes-sexto',  # represents the url text
                   name='Puntaje promedio en pruebas estandarizadas para alumnos de sexto grado',  # name of page, commonly used as name of link
                   title='Puntaje promedio en pruebas estandarizadas para alumnos de sexto grado'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_spanish/puntajes_6to.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total, matemática', id='page14_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page14_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page14_ninez_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-ninez-spanish', 'value'),
    Input('page14_ninez_spanish-comparacion_por_elect', 'value'),
