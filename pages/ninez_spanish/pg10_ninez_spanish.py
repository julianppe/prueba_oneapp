...
dash.register_page(__name__,
                   path='/participacion-actividades-cuidado-ninos-ninez',  # represents the url text
...
# page 1 data
df = pd.read_csv("datasets/ninez_spanish/cuidado_ninos.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", id='page10_ninez_spanish-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Ratio mujeres/hombres', id='page10_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page10_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page10_ninez_spanish-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page10_ninez_spanish-line', 'figure'),
    Input('page10_ninez_spanish-pais_elect', 'value'),
    Input('page10_ninez_spanish-comparacion_por_elect', 'value'),
    [Input('page10_ninez_spanish-the_year','value')]
)
