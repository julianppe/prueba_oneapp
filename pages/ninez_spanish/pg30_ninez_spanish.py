...
dash.register_page(__name__,
                   path='/sigue-intentando-6to',  # represents the url text
                   name="Alumnos de sexto grado que responden que si a 'Aunque las cosas no me resulten, sigo intentándolo'",  # name of page, commonly used as name of link
                   title="Alumnos de sexto grado que responden que si a 'Aunque las cosas no me resulten, sigo intentándolo'"  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_spanish/sigue_intentando_6to.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page30_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page30_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page30_ninez_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-ninez-spanish', 'value'),
    Input('page30_ninez_spanish-comparacion_por_elect', 'value'),
