...
dash.register_page(__name__,
                   path='/mujeres-10-bajo-sexto',  # represents the url text
                   name='Porcentaje de mujeres en el 10% más bajo de puntajes en sexto grado',  # name of page, commonly used as name of link
                   title='Porcentaje de mujeres en el 10% más bajo de puntajes en sexto grado'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_spanish/mujeres_p10_6to.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total, matemática', id='page20_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page20_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page20_ninez_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-ninez-spanish', 'value'),
    Input('page20_ninez_spanish-comparacion_por_elect', 'value'),
