...
dash.register_page(__name__,
                   path='/pide-ayuda-6to',  # represents the url text
                   name="Alumnos de sexto grado que responden que si a 'Pido ayuda al profesor cuando no entiendo lo que hay que hacer.'",  # name of page, commonly used as name of link
                   title="Alumnos de sexto grado que responden que si a 'Pido ayuda al profesor cuando no entiendo lo que hay que hacer.'"  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_spanish/pide_ayuda_6to.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page27_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page27_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page27_ninez_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-ninez-spanish', 'value'),
    Input('page27_ninez_spanish-comparacion_por_elect', 'value'),
