...
dash.register_page(__name__,
                   path='/puntajes-15',  # represents the url text
                   name='Puntaje promedio en pruebas estandarizadas para alumnos de 15 años',  # name of page, commonly used as name of link
                   title='Puntaje promedio en pruebas estandarizadas para alumnos de 15 años'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_spanish/puntajes_15.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", id='page15_ninez_spanish-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total, matemática', id='page15_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page15_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page15_ninez_spanish-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page15_ninez_spanish-line', 'figure'),
    Input('page15_ninez_spanish-pais_elect', 'value'),
    Input('page15_ninez_spanish-comparacion_por_elect', 'value'),
    [Input('page15_ninez_spanish-the_year','value')]
)
...
def update_graphs(pais_v, comparacion_por_v, years_chosen):
    ...
    if comparacion_por_v == 'Brecha mujeres - hombres, matemática':
        fig_line = px.line(dff, x='ano', y='valor', color='pais', error_y='valor_errorestandar_matem',
    ...
    elif comparacion_por_v == 'Brecha mujeres - hombres, lectura':
        fig_line = px.line(dff, x='ano', y='valor', color='pais', error_y='valor_errorestandar_lectu',
        symbol= 'desagregacion',
        labels=dict(ano="Año", valor="", pais="País", indicador="Indicador", desagregacion="Desagregación")).update_xaxes(type='category').update_layout(margin=dict(l=10, r=10, t=10, b=10))
    elif comparacion_por_v == 'Brecha mujeres - hombres, ciencia':
