...
dash.register_page(__name__,
                   path='/ratio-tercero',  # represents the url text
                   name='Ratio de puntajes en pruebas estandarizadas para alumnos de tercer grado',  # name of page, commonly used as name of link
                   title='Ratio de puntajes en pruebas estandarizadas para alumnos de tercer grado'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_spanish/ratio_3ro.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total, matemática', id='page16_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page16_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page16_ninez_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-ninez-spanish', 'value'),
    Input('page16_ninez_spanish-comparacion_por_elect', 'value'),
