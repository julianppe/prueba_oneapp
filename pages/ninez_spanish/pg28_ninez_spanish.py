...
dash.register_page(__name__,
                   path='/revisa-bien-6to',  # represents the url text
                   name="Alumnos de sexto grado que responden que si a 'Antes de entregar una tarea o examen lo reviso bien.'",  # name of page, commonly used as name of link
                   title="Alumnos de sexto grado que responden que si a 'Antes de entregar una tarea o examen lo reviso bien.'"  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_spanish/revisa_bien_6to.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page28_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page28_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page28_ninez_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-ninez-spanish', 'value'),
    Input('page28_ninez_spanish-comparacion_por_elect', 'value'),
