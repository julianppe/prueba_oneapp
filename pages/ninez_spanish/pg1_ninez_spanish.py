...
dash.register_page(__name__,
                   path='/tasa-escolarizacion-pre-primaria-5',  # '/' is home page and it represents the url
                   name='Tasa de escolarización pre-primaria (5 años)',  # name of page, commonly used as name of link
                   title='Tasa de escolarización pre-primaria (5 años)',  # title that appears on browser's tab
...
df = pd.read_csv("datasets/ninez_spanish/escolarizacion_preprimaria.csv")
...
layout = html.Div(
    [
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", className="bg-light", id='page1_ninez_spanish-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, className="bg-light", persistence=True, persistence_type='memory', value='Total', id='page1_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page1_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page1_ninez_spanish-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page1_ninez_spanish-line', 'figure'),
    Input('page1_ninez_spanish-pais_elect', 'value'),
    Input('page1_ninez_spanish-comparacion_por_elect', 'value'),
    [Input('page1_ninez_spanish-the_year','value')]
)
