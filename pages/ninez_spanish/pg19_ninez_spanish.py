...
dash.register_page(__name__,
                   path='/mujeres-10-bajo-tercero',  # represents the url text
                   name='Porcentaje de mujeres en el 10% más bajo de puntajes en tercer grado',  # name of page, commonly used as name of link
                   title='Porcentaje de mujeres en el 10% más bajo de puntajes en tercer grado'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_spanish/mujeres_p10_3ro.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total, matemática', id='page19_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page19_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page19_ninez_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-ninez-spanish', 'value'),
    Input('page19_ninez_spanish-comparacion_por_elect', 'value'),
...
def update_graphs(pais_v, comparacion_por_v, years_chosen):
    ...
    if comparacion_por_v == 'Brecha mujeres - hombres':
        ...
        labels=dict(ano="Año", valor="", pais2="País", indicador="Indicador", desagregacion="Desagregación")).update_xaxes(type='category', categoryorder='category ascending').update_layout(margin=dict(l=10, r=10, t=10, b=10))
