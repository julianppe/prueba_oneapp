...
# page 1 data
df = pd.read_csv("datasets/ninez_spanish/mujeres_p90_15.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", id='page24_ninez_spanish-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total, matemática', id='page24_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page24_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page24_ninez_spanish-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page24_ninez_spanish-line', 'figure'),
    Input('page24_ninez_spanish-pais_elect', 'value'),
    Input('page24_ninez_spanish-comparacion_por_elect', 'value'),
    [Input('page24_ninez_spanish-the_year','value')]
)
