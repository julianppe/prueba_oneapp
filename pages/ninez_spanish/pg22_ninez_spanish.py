...
dash.register_page(__name__,
                   path='/mujeres-10-alto-tercero',  # represents the url text
                   name='Porcentaje de mujeres en el 10% más alto de puntajes en tercer grado',  # name of page, commonly used as name of link
                   title='Porcentaje de mujeres en el 10% más alto de puntajes en tercer grado'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_spanish/mujeres_p90_3ro.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total, matemática', id='page22_ninez_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page22_ninez_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page22_ninez_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-ninez-spanish', 'value'),
    Input('page22_ninez_spanish-comparacion_por_elect', 'value'),
