...
dash.register_page(__name__,
                   path='/violence-physical-domestic',  # represents the url text
                   name='Percentage of women who have experienced physical violence by a partner',  # name of page, commonly used as name of link
                   title='Percentage of women who have experienced physical violence by a partner'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/gender_violence_english/violencia_fisica.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Bolivia", id='page2_gender_violence_english-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Women', id='page2_gender_violence_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page2_gender_violence_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page2_gender_violence_english-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page2_gender_violence_english-line', 'figure'),
    Input('page2_gender_violence_english-pais_elect', 'value'),
    Input('page2_gender_violence_english-comparacion_por_elect', 'value'),
    [Input('page2_gender_violence_english-the_year','value')]
)
