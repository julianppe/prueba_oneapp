...
dash.register_page(__name__,
                   path='/keep-trying-6th',  # represents the url text
                   name="Sixth grades students responding yes to 'Even if things don't work out for me, I keep trying'.",  # name of page, commonly used as name of link
                   title="Sixth grades students responding yes to 'Even if things don't work out for me, I keep trying'."  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ninez_english/sigue_intentando_6to.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total, mathematic', id='page30_ninez_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page30_ninez_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page30_ninez_english-line', 'figure'),
    Input('all-pages-dropdown-pais-ninez-english', 'value'),
    Input('page30_ninez_english-comparacion_por_elect', 'value'),
