...
# page 1 data
df = pd.read_csv("datasets/ninez_english/hs_cuidado.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", id='page9_ninez_english-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Women/men ratio', id='page9_ninez_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page9_ninez_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page9_ninez_english-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page9_ninez_english-line', 'figure'),
    Input('page9_ninez_english-pais_elect', 'value'),
    Input('page9_ninez_english-comparacion_por_elect', 'value'),
    [Input('page9_ninez_english-the_year','value')]
)
...
def update_graphs(pais_v, comparacion_por_v, Year_chosen):
    ...
    if comparacion_por_v == "Women - men gap":
        ...
        labels=dict(ano="Year", valor="", pais="Country", indicador="Indicator", desagregacion="Disaggregation")).update_xaxes(type='category').update_xaxes(type='category', categoryorder='category ascending').update_layout(margin=dict(l=10, r=10, t=10, b=10))
