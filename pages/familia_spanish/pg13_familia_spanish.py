...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page13_familia_spanish-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Ratio mujeres/hombres', id='page13_familia_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page13_familia_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page13_familia_spanish-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page13_familia_spanish-pais_elect', "value"),
    Output("store", "data"),
    Input('page13_familia_spanish-pais_elect', "value"),
    State("store", "data"),
)
...
@callback(
    Output('page13_familia_spanish-line', 'figure'),
    Input('page13_familia_spanish-pais_elect', 'value'),
    Input('page13_familia_spanish-comparacion_por_elect', 'value'),
    [Input('page13_familia_spanish-the_year','value')]
)
