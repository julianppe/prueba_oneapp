...
# page 1 data
df = pd.read_csv("datasets/familia_spanish/educado_inactivo.csv")
...
layout = html.Div([
        ...
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page5_familia_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page5_familia_spanish-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    ...
    Input('page5_familia_spanish-comparacion_por_elect', 'value'),
    [Input('page5_familia_spanish-the_year','value')]
)
