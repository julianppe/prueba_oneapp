...
dash.register_page(__name__,
                   path='/actividades-apoyo-familia',  # represents the url text
...
# page 1 data
df = pd.read_csv("datasets/familia_spanish/ayuda_otros.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", id='page16_familia_spanish-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Ratio mujeres/hombres', id='page16_familia_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page16_familia_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page16_familia_spanish-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page16_familia_spanish-line', 'figure'),
    Input('page16_familia_spanish-pais_elect', 'value'),
    Input('page16_familia_spanish-comparacion_por_elect', 'value'),
    [Input('page16_familia_spanish-the_year','value')]
)
