...
dash.register_page(__name__,
                   path='/pareja-homosexual',  # represents the url text
                   name='Porcentaje de personas con pareja del mismo sexo',  # name of page, commonly used as name of link
                   title='Porcentaje de personas con pareja del mismo sexo'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/familia_spanish/pareja_homosexual.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page35_familia_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page35_familia_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page35_familia_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-familia-spanish', 'value'),
    Input('page35_familia_spanish-comparacion_por_elect', 'value'),
