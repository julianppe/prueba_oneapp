...
dash.register_page(__name__,
                   path='/hijos-adultos',  # represents the url text
                   name='Adultos con hijos',  # name of page, commonly used as name of link
                   title='Adultos con hijos'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/familia_spanish/hijos_adultos.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page38_familia_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page38_familia_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page38_familia_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-familia-spanish', 'value'),
    Input('page38_familia_spanish-comparacion_por_elect', 'value'),
