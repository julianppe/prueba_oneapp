...
dash.register_page(__name__,
                   path='/metodo-anticonceptivos-sinacceso-mics',  # represents the url text
...
# page 1 data
df = pd.read_csv("datasets/familia_spanish/anticoncepcion_sin_acceso_mics.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Mujeres', id='page33_familia_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page33_familia_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page33_familia_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-familia-spanish', 'value'),
    Input('page33_familia_spanish-comparacion_por_elect', 'value'),
