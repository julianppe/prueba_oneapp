...
dash.register_page(__name__,
                   ...
                   name='Población adulta de acuerdo con el matrimonio entre personas del mismo sexo',  # name of page, commonly used as name of link
                   title='Población adulta de acuerdo con el matrimonio entre personas del mismo sexo'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/familia_spanish/matrimonio_igualitario.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Ratio mujeres/hombres', id='page36_familia_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page36_familia_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page36_familia_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-familia-spanish', 'value'),
    Input('page36_familia_spanish-comparacion_por_elect', 'value'),
