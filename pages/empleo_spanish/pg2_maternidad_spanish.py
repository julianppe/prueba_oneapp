...
dash.register_page(
    __name__,
    path='/empleo',  
    name='Efecto maternidad sobre empleo',  
    title='Efecto maternidad sobre empleo'
)
...
df = pd.read_csv("datasets/empleo_spanish/empleo_mat.csv")
...
layout = html.Div([
    dbc.Row([
        ...
        dbc.Col([
            dcc.Dropdown(
                ...
                id='page_maternidad_empleo-comparacion_por'
            )
        ], width=6),
    ]),
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(
                id='page_maternidad_empleo-line',
...
@callback(
    Output('page_maternidad_empleo-line', 'figure'),
    Input('all-pages-dropdown-pais-empleo-spanish', 'value'),
    Input('page_maternidad_empleo-comparacion_por', 'value'),
)
def update_graph(pais_v, comparacion_por_v):
    ...
    + "Cada punto representa el cambio porcentual en en la tasa de empleo en el "
