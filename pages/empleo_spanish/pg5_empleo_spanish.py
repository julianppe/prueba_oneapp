...
dash.register_page(__name__,
                   path='/horas-de-trabajo-ocup',  # represents the url text
                   name='Horas semanales en trabajo remunerado (solo ocupados)',  # name of page, commonly used as name of link
                   title='Horas semanales en trabajo remunerado (solo ocupados)'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/empleo_spanish/horas_trabajadas_ocup.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page5_empleo_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page5_empleo_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
# @callback(
#     Output('page5_empleo_spanish-pais_elect', "value"),
#     Output("store_empleo_spanish", "data"),
#     Input('page5_empleo_spanish-pais_elect', "value"),
#     State("store_empleo_spanish", "data"),
# )


@callback(
    Output('page5_empleo_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-empleo-spanish', 'value'),
    Input('page5_empleo_spanish-comparacion_por_elect', 'value'),
