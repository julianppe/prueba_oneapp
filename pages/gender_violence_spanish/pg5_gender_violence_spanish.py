...
from elements.elements_gender_violence_spanish import ranger_slider_year_gender_violence_spanish, generate_dropdown

dash.register_page(__name__,
                   path='/justifica-golpear',  # represents the url text
                   name='Porcentaje de la población que piensa que está justificado que los hombres golpeen a sus esposas',  # name of page, commonly used as name of link
                   title='Porcentaje de la población que piensa que está justificado que los hombres golpeen a sus esposas'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/gender_violence_spanish/justifica_golpear.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Mujeres', id='page5_gender_violence_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page5_gender_violence_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
            ranger_slider_year_gender_violence_spanish,
        ], width=12),
    ]),
])
...
@callback(
    Output('page5_gender_violence_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-gender-violence-spanish', 'value'),
    Input('page5_gender_violence_spanish-comparacion_por_elect', 'value'),
    [Input('all-pages-ranger-slider-year-gender-violence-spanish','value')]
)
