...
dash.register_page(__name__,
                   path='/justified-beat-mics',  # represents the url text
                   name='Percentage of women who think that husbands are justified to beat their wives in some situations (MICS)',  # name of page, commonly used as name of link
                   title='Percentage of women who think that husbands are justified to beat their wives in some situations (MICS)'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/gender_roles_english/justifica_golpear_mics.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Women', id='page20_gender_roles_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page20_gender_roles_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page20_gender_roles_english-line', 'figure'),
    Input('all-pages-dropdown-pais-gender-roles-english', 'value'),
    Input('page20_gender_roles_english-comparacion_por_elect', 'value'),
