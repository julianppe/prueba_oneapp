...
dash.register_page(__name__,
                   path='/learn-faster-6th-innate',  # represents the url text
...
# page 1 data
df = pd.read_csv("datasets/gender_roles_english/aprenden_rapido_6to_innato.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Gender, mathematics', id='page19_gender_roles_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page19_gender_roles_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page19_gender_roles_english-line', 'figure'),
    Input('all-pages-dropdown-pais-gender-roles-english', 'value'),
    Input('page19_gender_roles_english-comparacion_por_elect', 'value'),
