...
dash.register_page(__name__,
                   path='/woman-earns-more',  # represents the url text
                   name='Adult population agreeing with the statement “If a woman earns more money than her husband, it’s almost certain to cause problems”',  # name of page, commonly used as name of link
                   title='Adult population agreeing with the statement “If a woman earns more money than her husband, it’s almost certain to cause problems”'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/gender_roles_english/mujer_gana_mas.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", id='page12_gender_roles_english-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page12_gender_roles_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page12_gender_roles_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page12_gender_roles_english-the_year',
                min=2000,
                max=2021,
                value=[2000,2020],
                marks=mark_values,
...
@callback(
    Output('page12_gender_roles_english-line', 'figure'),
    Input('page12_gender_roles_english-pais_elect', 'value'),
    Input('page12_gender_roles_english-comparacion_por_elect', 'value'),
    [Input('page12_gender_roles_english-the_year','value')]
)
