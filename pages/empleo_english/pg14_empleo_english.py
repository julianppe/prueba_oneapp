...
# page 1 data
df = pd.read_csv("D:/Dropbox/multipage/projectGENLAC/dash_final_oneapp/datasets/empleo_english/anios_educ.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page14_empleo_english-pais_elect')
        ], width=6),
        dbc.Col([
        ...
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page14_empleo_english-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    ...
    Input('page14_empleo_english-pais_elect', 'value'),
    Input('page14_empleo_english-comparacion_por_elect', 'value'),
    [Input('page14_empleo_english-the_year','value')]
)
