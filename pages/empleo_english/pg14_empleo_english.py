...
dash.register_page(__name__,
                   path='/gender-wage-gap',  # represents the url text
                   name='Gender wage gap (conditional)',  # name of page, commonly used as name of link
                   title='Gender wage gap (conditional)'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/empleo_english/brecha_salarial_genero.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page14_empleo_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page14_empleo_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    ...
    Input('page14_empleo_english-comparacion_por_elect', 'value'),
