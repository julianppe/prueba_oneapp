...
import pandas as pd
from elements.elements_empleo_english import dropdown_pais_empleo_english, ranger_slider_year_empleo_english
from dash import dcc, html, register_page, ctx, no_update
from dash_extensions.enrich import Output, Input, State, callback

dash.register_page(__name__,
                   path='/weekly-hours-worked-employed',  # represents the url text
                   name='Weekly hours worked (only employed)',  # name of page, commonly used as name of link
                   title='Weekly hours worked (only employed)'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/empleo_english/horas_trabajadas_ocup.csv")
...
layout = html.Div([
        ...
        dbc.Row([
        dbc.Col([
            ranger_slider_year_empleo_english
        ], width=12),
    ]),
])
