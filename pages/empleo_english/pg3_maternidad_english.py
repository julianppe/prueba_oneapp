...
dash.register_page(
    __name__,
    path='/hours_worked',  
    name='Motherhood effect on hours worked',  
    title='Motherhood effect on hours worked'
)
...
df = pd.read_csv("datasets/empleo_english/hours_worked_mat.csv")
...
layout = html.Div([
    dbc.Row([
        ...
        dbc.Col([
            dcc.Dropdown(
                ...
                id='page_maternidad_hours_worked-comparacion_por'
            )
        ], width=6),
    ]),
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(
                id='page_maternidad_hours_worked-line',
...
@callback(
    Output('page_maternidad_hours_worked-line', 'figure'),
    Input('all-pages-dropdown-pais-empleo-english', 'value'),
Input('page_maternidad_hours_worked-comparacion_por', 'value')
)
def update_graph(pais_v, comparacion_por_v):
    ...
    + "Each point represents the percentage change in weekly hours worked in the "
