...
dash.register_page(__name__,
                   path='/labor-force-participation',  # '/' is home page and it represents the url
                   name='Labor force participation rate',  # name of page, commonly used as name of link
                   title='Labor force participation rate',  # title that appears on browser's tab
...
df = pd.read_csv("E:/Dropbox/multipage/projectGENLAC/dash_final_oneapp/datasets/empleo_english/participacion.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", className="bg-light", id='page1_empleo_english-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, className="bg-light", persistence=True, persistence_type='memory', value='Total', id='page1_empleo_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
...
#@callback(
#    Output('page1_empleo_english-pais_elect', 'value'),
