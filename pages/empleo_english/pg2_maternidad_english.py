...
dash.register_page(
    __name__,
    path='/employment',  
    name='Motherhood effect on employment',  
    title='Motherhood effect on employment'
)
...
df = pd.read_csv("datasets/empleo_english/employment_mat.csv")
...
layout = html.Div([
    dbc.Row([
        ...
        dbc.Col([
            dcc.Dropdown(
                ...
                id='page_maternidad_employment-comparacion_por'
            )
        ], width=6),
    ]),
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(
                id='page_maternidad_employment-line',
...
@callback(
    Output('page_maternidad_employment-line', 'figure'),
    Input('all-pages-dropdown-pais-empleo-english', 'value'),
    Input('page_maternidad_employment-comparacion_por', 'value')
)
def update_graph(pais_v, comparacion_por_v):
    ...
    + "Each point represents the percentage change in employment in the "
