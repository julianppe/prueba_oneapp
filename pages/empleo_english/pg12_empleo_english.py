...
# page 1 data
df = pd.read_csv("D:/Dropbox/multipage/projectGENLAC/dash_final_oneapp/datasets/empleo_english/ingreso_laboral.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page12_empleo_english-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page12_empleo_english-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page12_empleo_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page12_empleo_english-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page12_empleo_english-line', 'figure'),
    Input('page12_empleo_english-pais_elect', 'value'),
    Input('page12_empleo_english-comparacion_por_elect', 'value'),
    [Input('page12_empleo_english-the_year','value')]
)
