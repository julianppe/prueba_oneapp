...
dash.register_page(__name__,
                   path='/employment-rate',  # represents the url text
                   name='Employment rate',  # name of page, commonly used as name of link
                   title='Employment rate'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("E:/Dropbox/multipage/projectGENLAC/dash_final_oneapp/datasets/empleo_english/empleo.csv")
...
layout = html.Div([
        ...
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page2_empleo_english-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page2_empleo_english-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    ...
    Input('page2_empleo_english-pais_elect', 'value'),
    Input('page2_empleo_english-comparacion_por_elect', 'value'),
    [Input('page2_empleo_english-the_year','value')]
)
