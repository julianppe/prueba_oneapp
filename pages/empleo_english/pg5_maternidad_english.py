...
dash.register_page(
    __name__,
    path='/informality',  
    name='Motherhood effect on informality',  
    title='Motherhood effect on informality'
)
...
df = pd.read_csv("datasets/empleo_english/informality_mat.csv")
...
layout = html.Div([
    dbc.Row([
        ...
        dbc.Col([
            dcc.Dropdown(
                ...
                id='page_maternidad_informality-comparacion_por'
            )
        ], width=6),
    ]),
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(
                id='page_maternidad_informality-line',
...
@callback(
    Output('page_maternidad_informality-line', 'figure'),
    Input('all-pages-dropdown-pais-empleo-english', 'value'),
    Input('page_maternidad_informality-comparacion_por', 'value')
)
def update_graph(pais_v, comparacion_por_v):
    ...
    + "Each point represents the percentage change in the labor informality rate in the "
