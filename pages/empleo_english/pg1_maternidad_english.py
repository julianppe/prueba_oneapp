...
# IMPORTAR ELEMENTOS DE EMPLEO (MODIFIQUE)
from elements.elements_empleo_english import (
...
dash.register_page(
    __name__,
    path='/participation',  
    name='Motherhood effect on labor participation',  
    title='Motherhood effect on labor participation'
)
...
df = pd.read_csv("datasets/empleo_english/participation_mat.csv")
...
layout = html.Div([
    dbc.Row([
        dbc.Col([
            dropdown_pais_empleo_english, # MODIFIQUE
        ], width=6),
        dbc.Col([
            dcc.Dropdown(
                ...
                id='page_maternidad_participation-comparacion_por'
            )
        ], width=6),
    ]),
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(
                id='page_maternidad_participation-line',
    ...
    #dbc.Row([
    #    dbc.Col([
    #        ranger_slider_year_maternidad_english, 
    #    ], width=12),
    #]),
])
...
@callback(
    Output('page_maternidad_participation-line', 'figure'),
    Input('all-pages-dropdown-pais-empleo-english', 'value'), # MODIFIQUE
    Input('page_maternidad_participation-comparacion_por', 'value')
    #Input('all-pages-ranger-slider-year-maternidad-english', 'value') 
)
#def update_graph(pais_v, comparacion_por_v, years_chosen):
def update_graph(pais_v, comparacion_por_v):
    ...
    detalle_largo = (
    "The graph shows the effects of first childbirth (with 95% confidence intervals) "
    "estimated from an event-study model using individual-level pseudo-panels.<br>"
    + "Each point represents the percentage change in labor force participation in the "
    "indicated year relative to the year prior to childbirth (normalized to zero), "
    "controlling for cohort and year fixed effects."
    )
    ...
    fig = px.line(
        ...
        labels=dict(
            ano="Years relative to childbirth",
            gap="",
            pais2="Country",
            desagregacion="Disaggregation"
        )
    )
    ...
    annotations = [
        ...
        dict(
            xref='paper', yref='paper', x=0.001, y=-0.44,
            xanchor='left', yanchor='bottom',
            text=disclaimer,
