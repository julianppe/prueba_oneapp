...
dash.register_page(
    __name__,
    path='/labor_income',  
    name='Motherhood effect on labor income',  
    title='Motherhood effect on labor income'
)
...
df = pd.read_csv("datasets/empleo_english/labor_income_mat.csv")
...
layout = html.Div([
    dbc.Row([
        ...
        dbc.Col([
            dcc.Dropdown(
                ...
                id='page_maternidad_labor_income-comparacion_por'
            )
        ], width=6),
    ]),
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(
                id='page_maternidad_labor_income-line',
...
@callback(
    Output('page_maternidad_labor_income-line', 'figure'),
    Input('all-pages-dropdown-pais-empleo-english', 'value'),
    Input('page_maternidad_labor_income-comparacion_por', 'value')
)
def update_graph(pais_v, comparacion_por_v):
    ...
    + "Each point represents the percentage change in monthly labor income in the "
