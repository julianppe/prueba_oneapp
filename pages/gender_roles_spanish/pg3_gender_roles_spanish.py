...
dash.register_page(__name__,
                   path='/derecho-empleo',  # represents the url text
                   name='Población adulta de acuerdo con la frase “ante la escasez de empleo, los hombres deberían tener más derecho a un empleo que las mujeres”',  # name of page, commonly used as name of link
                   title='Población adulta de acuerdo con la frase “ante la escasez de empleo, los hombres deberían tener más derecho a un empleo que las mujeres”'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/gender_roles_spanish/derecho_empleo.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", id='page3_gender_roles_spanish-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page3_gender_roles_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page3_gender_roles_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page3_gender_roles_spanish-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page3_gender_roles_spanish-line', 'figure'),
    Input('page3_gender_roles_spanish-pais_elect', 'value'),
    Input('page3_gender_roles_spanish-comparacion_por_elect', 'value'),
    [Input('page3_gender_roles_spanish-the_year','value')]
)
...
def update_graphs(pais_v, comparacion_por_v, years_chosen):
    ...
    if comparacion_por_v == 'Brecha mujeres - hombres':
        fig_line = px.bar(dff, x='ano', y='valor', color='pais', error_y='valor_errorestandar', pattern_shape='desagregacion', 
