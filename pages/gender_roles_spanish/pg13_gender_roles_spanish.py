...
dash.register_page(__name__,
                   path='/madre-trabaja',  # represents the url text
                   name='Población adulta de acuerdo con la frase “cuando una madre trabaja, sus hijos sufren”',  # name of page, commonly used as name of link
                   title='Población adulta de acuerdo con la frase “cuando una madre trabaja, sus hijos sufren”'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/gender_roles_spanish/madre_trabaja.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page13_gender_roles_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page13_gender_roles_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page13_gender_roles_spanish-line', 'figure'),
    Input('all-pages-dropdown-pais-gender-roles-spanish', 'value'),
    Input('page13_gender_roles_spanish-comparacion_por_elect', 'value'),
