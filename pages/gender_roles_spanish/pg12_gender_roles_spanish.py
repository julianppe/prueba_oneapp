...
dash.register_page(__name__,
                   path='/mujer-gana-mas',  # represents the url text
                   name='Población adulta de acuerdo con la frase “si una mujer gana más dinero que su marido, es casi seguro que causará problemas”',  # name of page, commonly used as name of link
                   title='Población adulta de acuerdo con la frase “si una mujer gana más dinero que su marido, es casi seguro que causará problemas”'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/gender_roles_spanish/mujer_gana_mas.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, value="Argentina", id='page12_gender_roles_spanish-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page12_gender_roles_spanish-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page12_gender_roles_spanish-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page12_gender_roles_spanish-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page12_gender_roles_spanish-line', 'figure'),
    Input('page12_gender_roles_spanish-pais_elect', 'value'),
    Input('page12_gender_roles_spanish-comparacion_por_elect', 'value'),
    [Input('page12_gender_roles_spanish-the_year','value')]
)
